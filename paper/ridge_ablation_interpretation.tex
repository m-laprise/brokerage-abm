...
size-matched broker-minus-principal difference in holdout rank correlation is
\rav{size_matchedBaseline_rank_gap}, a paired change of
\rav{size_matchedBaselineDelta_rank_gap} from base Ridge (95\% Monte Carlo
...
The size-matched contrast is consistent across the design. Its
broker-minus-principal difference in holdout rank correlation is lower than
base Ridge in all \rav{nConditions} effective realizations. The
...
\rav{size_matchedDeltaQ90_rank_gap}. The difference nevertheless remains positive in
\rav{size_matchedPositiveN_rank_gap}/\rav{nConditions} realizations. The change
in this difference is not solely a change in the broker's prediction. The median
broker holdout rank correlation changes by
\rav{size_matchedDeltaMedian_broker_holdout_rank}, while the median principal
holdout rank correlation changes by
\rav{size_matchedDeltaMedian_agent_holdout_rank}. The principal holdout rank correlation
also exceeds base Ridge in all \rav{size_matchedGreaterN_agent_holdout_rank}
...
when the interaction component of the production function is strong. The median
additive-Ridge broker-minus-principal difference in holdout rank correlation
rises from
...
function, additive Ridge exceeds the corresponding base Ridge difference of
\rav{pairRho1_rank_gap}. The latter result is consistent with a finite-sample
cost from unnecessary bilinear regressors, but this contrast alone
...
a pair score by adding two fitted one-type scores. At the baseline, subtracting
the additive variant's broker-minus-principal difference in holdout rank
correlation from the single-principal variant's gives
...
\rav{nConditions} realizations under each ablation, even when the
broker-minus-principal difference in holdout rank correlation is negative. This
channel mean difference is descriptive, not a causal
