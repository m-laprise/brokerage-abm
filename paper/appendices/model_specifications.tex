...
The predictable match value \(f\) is fully determined by the two types and by matching-function objects fixed at initialization. The term \(\varepsilon_{ij}\) is random match variation.
...
accounts for this small covariance. Averaged across the independently generated environments, each standardized component and the resulting predictable match value \(f\) have within-market variance one for every \(\rho\) and \(\delta\). This defines the payoff unit. In these units, \(Q=2.84\) and \(\sigma_\varepsilon=0.28\).
