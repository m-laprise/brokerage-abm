...
Each pair's match value combines two sources: the agents' average general quality and a match-specific complementarity between their types. The parameter \(\rho\) moves the matching problem from pure complementarity to pure general quality. The parameter \(\delta\) makes complementarity harder to predict by increasing the difference between otherwise similar pairs on opposite sides of a boundary in type-pair space. Match value is symmetric in the two agents.

\subsubsection{2a. Match Quality}\label{a.-match-quality}

Let \(q_{ij}\) be the per-period value of the match between agents \(i\) and \(j\), measured in payoff units:

\[q_{ij} = Q + f(\mathbf{x}_i, \mathbf{x}_j) + \varepsilon_{ij}, \qquad
\varepsilon_{ij} \sim \mathcal{N}(0, \sigma_\varepsilon^2).\]

The predictable match value \(f\) is fully determined by the two types and by matching-function objects fixed at initialization. The term \(\varepsilon_{ij}\) is random match variation. The payoff unit is chosen so that, averaging across fixed reference markets, the within-market variance of \(f\) across possible pairs is one (§2d). In these units, \(Q=2.840698\) is a constant offset and \(\sigma_\varepsilon=0.284070\).
...
Here \(\widetilde U_{ij}\) is normalized general quality (§2b) and \(\widetilde W_{ij}(\delta)\) is normalized match-specific complementarity (§2c). Under the reference moments, \(\rho\in[0,1]\) is general quality's share of the two components' summed marginal variances; \(1-\rho\) is complementarity's share. Thus \(\rho=0\) gives pure complementarity and \(\rho=1\) gives pure general quality. At \(\rho=1\), \(\delta\) has no effect. The common factor \(s_{\mathrm{mix}}(\rho,\delta)\) accounts for the components' small covariance and keeps the average within-market variance of \(f\) fixed (§2d).
...
\mathbf c=\boldsymbol{\gamma}(u_c)+\boldsymbol{\epsilon}_c.\]
...
The trace normalization prevents arbitrary scale differences in \(\mathbf{A}\) across realizations. The pair-level reference moments in §2d then place complementarity on the common scale used in the matching function.
...
\subsubsection{2d. Fixed Component Normalization}\label{d.-fixed-component-normalization}

The component moments are computed once from reference markets and then held fixed in every simulation. They are not re-estimated within a simulated market. This preserves ordinary between-market variation instead of forcing every realization to have exactly the same component means and variances.

The reference set contains 50 independently generated matching environments with \(N=1000\), \(d=d_\gamma=8\), and \(\sigma_x=0.5\), using seeds 10,001 through 10,050, which are separate from the sweep seeds. Each environment contains newly drawn types, \(\mathbf c\), \(\mathbf A\), and \(\mathbf B\), but no simulation periods or random match variation. Means, variances, and covariances are computed over all unordered pairs of distinct agents within each market, then averaged across markets with equal weight. The resulting reference moments are

\[
\begin{aligned}
\mu_U&=0.0026670734, & \sigma_U^2&=0.1076132586,\\
\mu_V&=0.0002323259, & \sigma_V^2&=0.3135097944,\\
\mu_H&=-0.0012126664, & \sigma_H^2&=0.3131030983,
\end{aligned}
\]

with \(\operatorname{Cov}(U,V)=0.0006659568\), \(\operatorname{Cov}(U,H)=-0.0025342379\), and \(\operatorname{Cov}(V,H)=-0.0025068532\). Therefore

\[
\begin{aligned}
\mu_W(\delta)&=\mu_V+\delta\mu_H,\\
\sigma_W^2(\delta)&=\sigma_V^2+2\delta\operatorname{Cov}(V,H)+\delta^2\sigma_H^2,\\
\kappa(\delta)&=\frac{\operatorname{Cov}(U,V)+\delta\operatorname{Cov}(U,H)}{\sigma_U\sigma_W(\delta)},\\
s_{\mathrm{mix}}^2(\rho,\delta)&=1+2\sqrt{\rho(1-\rho)}\,\kappa(\delta).
\end{aligned}
\]

These definitions make the average within-market variance of both \(\widetilde U\) and \(\widetilde W(\delta)\) equal to one over the reference markets. The common correction \(s_{\mathrm{mix}}\) accounts for their average within-market covariance, making the corresponding average variance of \(f\) equal to one for every \(\rho\) and \(\delta\). Because it scales both components equally, it does not alter the balance between general quality and complementarity set by \(\rho\). The small covariance cannot be assigned uniquely to either component, so \(\rho\) allocates only their marginal variances. A new simulated market is not rescaled to match these moments exactly, so its component means and variances may differ.
...
where \(n_{\text{new}}\) is the number of observations added since that model's previous fitting call, \(n_{\text{total}} = |\mathcal{H}^t|\) is full history size, and \(E_{\text{steps},c}\) is the learner-specific minimum per-call step count. For an agent, \(n_{\text{new}}\) can span multiple periods because agents fit on an alternating schedule. Agents and the broker use the same functional rule but may have different parameter values. Ridge is solved directly at each fitting call. Reporting values for the NN learning rates and update budgets are selected separately at the baseline using out-of-sample rank correlation and calibration seeds excluded from the reported simulations; the complete calibration is preserved in the replication code.
...
\(\sigma_\varepsilon\) & 0.284 & §2a & \(\alpha_R\) & 0.20 & §6b \\
\(\eta_{\mathrm{lr},a},\eta_{\mathrm{lr},b}\) & 0.01, 0.01 & §3a & \(p_{\text{roster}}\) & 0.02 & §6b \\
\(E_{\text{init},a},E_{\text{init},b}\) & 200, 200 & §3a & \(\omega\) & 0.2 & §7a \\
\(E_{\text{steps},a},E_{\text{steps},b}\) & 100, 100 & §3a & \(\Delta_{\mathrm{net}}\) & 20 & §8 \\
