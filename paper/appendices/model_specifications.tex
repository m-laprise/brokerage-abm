...
The model is a discrete-time simulation of a matching market with \emph{agents} and a \emph{broker}. Agents seeking pairwise matches either search on their own or outsource to the broker. All economic quantities (match output, fees, surplus) are in the same payoff units. All agents use heuristic decision rules. No agent holds beliefs about other agents' strategies.

The model specification covers agents (§1), the matching problem (§2), how agents learn to predict expected match output (§3), match economics (§4), network structure and agent turnover (§5), search and broker access (§6), the outsourcing decision (§7), and period measures (§8). Baseline parameter values and reproducibility are documented in Part II (§9).
...
The frequencies \(\nu_\ell \sim \operatorname{Unif}\{1,2,3,4,5\}\) and phases \(\psi_\ell \sim \operatorname{Unif}[0,2\pi)\) are drawn once per simulation. The parameter \(d_\gamma\leq d\) is the number of coordinates in which the curve varies.
...
Each potential pair's expected match output combines two sources: the agents' average general quality and a match-specific complementarity between their types. The parameter \(\rho\) moves the matching problem from pure complementarity to pure general quality. The parameter \(\delta\) makes complementarity harder to predict by increasing the difference between otherwise similar pairs on opposite sides of a boundary in type-pair space. Expected and realized match output are symmetric in the two agents.

\subsubsection{2a. Match Output}\label{a.-match-output}

Let \(q_{ij}\) be the realized per-period output of a match between agents \(i\) and \(j\), measured in payoff units:

\[q_{ij} = Q + f(\mathbf{x}_i, \mathbf{x}_j) + \varepsilon_{ij}, \qquad
\varepsilon_{ij} \sim \mathcal{N}(0, \sigma_\varepsilon^2).\]

The conditional expected output is \(E[q_{ij}\mid\mathbf{x}_i,\mathbf{x}_j]=Q+f(\mathbf{x}_i,\mathbf{x}_j)\). The systematic component \(f\) is fully determined by the two types and by matching-function objects fixed at initialization. The independent, mean-zero shock \(\varepsilon_{ij}\) creates match-specific variation that neither the agents nor the broker can predict.

The function \(f: \mathbb{R}^d \times \mathbb{R}^d \to \mathbb{R}\) is unknown to all agents:
...
The tildes indicate that general quality (§2b) and match-specific complementarity (§2c) are centered and scaled using fixed moments. These moments are estimated once from independently generated matching environments. Within each environment, they are calculated across all unordered pairs of distinct agents; they are then averaged across environments and held fixed in every simulation. Individual markets are not rescaled, so they retain ordinary between-market variation.

Let \(\kappa(\delta)\) denote the fixed covariance between general quality and complementarity after each component is standardized. The common adjustment

\[s_{\mathrm{mix}}^2(\rho,\delta)=1+2\sqrt{\rho(1-\rho)}\,\kappa(\delta)\]

accounts for this small covariance. Averaged across the independently generated environments, each standardized component and the resulting systematic component \(f\) have within-market variance one for every \(\rho\) and \(\delta\). Because \(Q\) is constant, expected match output has the same variance. This defines the payoff unit. In these units, \(Q=2.84\) and \(\sigma_\varepsilon=0.28\).

Because the two components have the same marginal variance on this scale, \(\rho\in[0,1]\) is general quality's share of their summed marginal variances, while \(1-\rho\) is complementarity's share. Thus \(\rho=0\) gives pure complementarity and \(\rho=1\) gives pure general quality. At \(\rho=1\), \(\delta\) has no effect.

\subsubsection{2b. Agent General Quality}\label{b.-agent-general-quality}

\(\mathbf{c}\) is an ideal type vector. An agent's general-quality score, \(\mathbf{x}_i^\top\mathbf{c}\), measures its alignment with that ideal. Pair-level general quality is the average of the two agents' scores:
...
The ideal type \(\mathbf{c}\in\mathbb{R}^d\) is drawn at initialization as a perturbation of a random point on the agent type curve:
...
The normalization places \(\mathbf c\) on the same unit sphere as agent types. The perturbation therefore changes the ideal direction without changing its magnitude.
...
The multiplier is \(1+\delta\) when \(\mathbf{x}_i^\top\mathbf{B}\mathbf{x}_j>0\), \(1-\delta\) when it is negative, and 1 when it is zero. At \(\delta=0\), the boundary between the two regions has no effect. For \(\delta>0\), pairs on opposite sides of the boundary can have sharply different expected output even when their types are otherwise similar. Increasing \(\delta\) increases this difference, making complementarity harder to learn. Normalizing \(W_{ij}(\delta)\) separately for each \(\delta\) removes the accompanying change in dispersion. Thus \(\delta\) changes the shape and difficulty of complementarity, not its variance.

\textbf{Boundary construction.} Matrix \(\mathbf{B}\) is made orthogonal to \(\mathbf{A}\) over the realized type distribution. This ensures that \(\delta\) introduces a distinct boundary rather than merely amplifying the base complementarity pattern. Let
...
This gives \(\operatorname{tr}(\boldsymbol{\Sigma}_x\mathbf{B}\boldsymbol{\Sigma}_x\mathbf{A})=0\) and \(\|\mathbf{B}\|_F=1\). Weighting by \(\boldsymbol{\Sigma}_x\) removes overlap between the two bilinear scores where agents actually occur, rather than in unused regions of \(\mathbb{R}^d\).
...
For every unordered pair \(\{i,j\}\) such that one side is a broker-client demander and the other side is in \(\mathcal{A}_b^t\), the broker computes predicted match output. For each broker-client demander \(i\), it ranks accessible counterparties above \(r\), breaking equal predictions uniformly at random, and keeps the highest-ranked candidates up to \(i\)'s residual active broker demand. Directed broker offers are then processed in a global score ranking with a new, independent random ordering within score ties. If both sides are broker-client demanders with remaining active demand, the same unordered pair can generate reciprocal broker offers.

The broker assessment vs access experiment separates access from assessment. The default \textbf{full-service broker} uses the broker access set \(\mathcal A_b^t\) and the broker's pair predictions. The \textbf{assessment-only broker} applies the broker's pair predictions to the same candidate set an agent could reach through self-search: its network neighbors and an independently drawn stranger pool. The \textbf{access-only broker} presents candidates from \(\mathcal A_b^t\), but the client orders them using its own assessments: its pair-specific mean for an experienced neighbor and its prediction model otherwise. In every mode, candidates below the score source's reservation threshold are excluded.

These restrictions change only how brokered candidates are obtained and ordered. The broker continues to observe mediated outcomes and refit its model in all modes, although its predictions do not determine access-only offers. Offer acceptance, match formation, fees, satisfaction, reputation, learning histories, and network updating are unchanged. ``Assessment-only'' means that the broker adds no candidate access beyond ordinary self-search. It does not imply that every resulting match joins existing neighbors, because ordinary self-search includes strangers. Likewise, access-only service can select a current neighbor when that neighbor belongs to the broker access set.
...
Each agent \(i\) maintains a satisfaction index \(s_{i,c}^t\) for each search channel \(c \in \{\text{self}, \text{broker}\}\). For the chosen channel, the index updates once per active-demand period as an EWMA of realized match output net of search costs:
...
Prediction quality is evaluated on random candidate pairs and accepted directed offers:
...
\bottomrule\noalign{}
\endlastfoot
Random candidate pairs & Up to 100 agents with non-empty histories; for each sampled agent \(i\), \(\min(40,N-1)\) non-self partners \(j\). Agent and broker predictions are evaluated against noiseless \(Q + f(\mathbf{x}_i,\mathbf{x}_j)\). Both models use the same sample, which is deterministic conditional on seed and period. & Per-agent \(R^2\), RMSE, bias, and Spearman rank correlation, averaged across sampled agents. \\
...
Rank correlation evaluates the strict ranking produced from predicted output. Candidates are ordered by predicted output, with each set of equal predictions ordered uniformly at random. Agent and broker tie-breaking draws are independent. For random candidate pairs, Spearman's \(\rho_S\) compares this produced ranking with expected-output ranks. For selected offers, it compares the produced ranking with realized-output ranks. Equal target values retain their average ranks. Thus a constant predictor produces a random ranking with expected rank correlation zero. In the definitions above, \(q\) denotes the relevant target: expected output for random candidate pairs and realized output for selected offers. For selected-offer summaries, \(R^2\), bias, and rank correlation are undefined when \(n < 5\) or \(\operatorname{Var}_{\mathrm{pop}}(q) < \sigma_\varepsilon^2/6\), while RMSE is defined whenever \(n>0\). Random-candidate summaries use common valid-agent support for all four measures: an agent-level candidate set that fails the sample-size or variance condition is excluded from the \(R^2\), RMSE, bias, and rank-correlation means.
...
Requested-position outcomes & For each channel, record the share of requested positions filled and realized output net of channel costs per requested position. Market-wide net output sums the two channel numerators and divides by all requested positions. Unfilled positions contribute zero output and remain in the denominator. \\
...
\(\sigma_\varepsilon\) & 0.28 & §2a & \(\alpha_R\) & 0.20 & §6b \\
\(\eta_{\mathrm{lr},a},\eta_{\mathrm{lr},b}\) & 0.003, 0.03 & §3a & \(p_{\text{roster}}\) & 0.02 & §6b \\
\(E_{\text{init},a},E_{\text{init},b}\) & 100, 100 & §3a & \(\omega\) & 0.2 & §7a \\
\(E_{\text{steps},a},E_{\text{steps},b}\) & 50, 50 & §3a & \(\Delta_{\mathrm{net}}\) & 20 & §8 \\
...
All model-event randomness flows from a single integer seed. The seed determines type draws, \(G\), matching function parameters (\(\mathbf{c}\), \(\mathbf{A}\), \(\mathbf{B}\)), broker seed roster, standing-roster churn and replenishment, random ordering within market score ties, and subsequent model events. Random candidate samples and their independent agent and broker tie-breaking draws are deterministic conditional on seed and period. Simulations are fully reproducible given parameters and seed.
