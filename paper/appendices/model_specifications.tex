...
\textbf{Assessment-only.} The broker scores the agent's network neighbors and an independently sampled stranger pool (§6a) using its pair predictions.

\textbf{Access-only.} Candidates come from the broker access set \(\mathcal A_b^t\). Scores use the requesting agent's pair-specific mean for experienced neighbors and its prediction model otherwise (§6a).
