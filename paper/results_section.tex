...
% Built by scripts/paper/build_section.jl on 2026-06-11 15:45
% from section_source.tex + captions.tex + values.tex + figmeta.tex;
% sweep 2026-06-07_f424438; repo commit 7eb888f.
...
\caption{Broker betweenness centrality and access fraction. Top row: ensemble means over
time at the baseline regime pair (no capture, blue; capture, orange), as
5-observation rolling means; betweenness is measured every
20 periods. Bottom: late means across the one-at-a-time no-capture
regimes, one dot per regime, colored by the late-mean output gap (broker minus
self). Axes start at $t=30$ (end of burn-in).}
\label{fig:position}
\end{figure}
...
\caption{Late means of four outcomes against the effective rank $r_{90}$ of the matching
...
\caption{Late means of nine outcomes across the $\rho\times\delta$ grid (no capture;
...
\caption{Baseline dynamics over time without capture (top row) and with capture (bottom
