...
**Fitting.** Each period, the network weights are updated by minimizing MSE with the Adam optimizer (learning rate $\eta_{\mathrm{lr}}$, default 0.01) on the current training window. Gradients are computed by automatic differentiation through DifferentiationInterface with Enzyme. Adam rather than plain gradient descent is essential for the broker: its symmetric pair features make the MSE landscape ill-conditioned, so a single global step size learns the high-variance quality and complementarity directions while starving the low-curvature regime-gain direction; Adam's per-parameter scaling recovers the gain. No explicit weight regularization is applied.
...
where $n_{\text{new}}$ is the number of observations added this period and $n_{\text{total}} = |\mathcal{H}^t|$ is the full history size. Using full history (not the training window) keeps the step count independent of the window and cap. The floor $E_{\text{steps}}$ (default 100) is the minimum per-period step count, reached once history dominates the ratio; it is a tuned cost/benefit knob — too few steps under-fit the interaction, while more steps give diminishing returns at linear cost. Agents and the broker use the same schedule, so their advantage gap reflects data, not optimization.

**Training window.** Each training period uses the observations recorded in the most recent $W_p = 40$ periods, *not* a fixed observation count. Defining the horizon in periods gives every learner the same wall-clock look-back regardless of match throughput; a fixed observation count would give the high-throughput broker a sub-one-period memory while a low-throughput agent's window spanned its entire history. Because the data-generating process is stationary, a longer horizon is never stale — it simply gives the broker more (and, from its earlier less-selected matches, better-balanced) examples of the regime; as $W_p \to T$ the window recovers all-history training. The broker's window can hold tens of thousands of matches, so it is subsampled uniformly across its span to at most $M = 2000$ observations, bounding training cost while preserving coverage. The warm start preserves what was learned from older data.
...
| $W_p$ | Training window (periods) | 40 | Train on observations from the most recent $W_p$ periods, subsampled to at most $M = 2000$ |
