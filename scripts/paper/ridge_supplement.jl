...
const PROVENANCE = manuscript_git_provenance(REPO)
...
function assessment_values()
    ...
    validate_analysis_commit(PROVENANCE, nn_meta["analysis_git_commit"]; artifact="NN figure data") ==
        DATA_ANALYSIS_COMMIT || error("NN and Ridge figure analysis commits differ")
    ...
    return (; values, comparison_commit)
end
