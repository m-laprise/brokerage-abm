...
const MAIN_FIGDATA = normpath(joinpath(@__DIR__, "..", "..", "output", "main", "figure_data.jld2"))
const FD = JLD2.load(MAIN_FIGDATA)["figdata"]
...
elseif isempty(ARGS)
    ...
    open(normpath(joinpath(@__DIR__, "..", "..", "output", "main", "figmeta.tex")), "w") do io
        ...
        println(io, "% Ridge ablation analysis commit: $INFORMATION_ANALYSIS_COMMIT")
        ridge_path = normpath(joinpath(OUT, "..", "..", "ridge", "paired", "figure_data.jld2"))
        println(io, "% Base Ridge analysis commit: ",
            JLD2.load(ridge_path)["figdata"]["meta"]["analysis_git_commit"])
        for path in (MAIN_FIGDATA, ridge_path, INFORMATION_FIGDATA)
            println(io, "% Input SHA256: ", relpath(path, REPORTING_PROVENANCE.root),
