...
scripts/paper/figdata.jl: it reads the full sweep (TB_SWEEP_DIR) and writes
...
Contents of supp_figdata.jld2 (single key "figdata", a Dict):
  ...
  series      per-period ensemble means at the baseline pair, per model
              (base, capture): constraint, effective_size (full per-period
              vectors; the renderer keeps the measurement periods)
  oat_cells   late means per one-at-a-time no-capture regime: access,
  ...
  base_cells  rho, constraint, effsize, rankgap, qgap per no-capture regime, for S3
  meta        sweep id, generation time, generating script

Usage: TB_SWEEP_DIR=<sweep root> julia --project scripts/paper/supp_figdata.jl
"""
...
qgap(m) = seedmean(m, :q_broker_standard_mean) - seedmean(m, :q_self_mean)
...
fd["oat_cells"] = let out = Dict{String,Float64}[]
    for rel in cellspec
        ...
        m = load_mdfs(rel)
        a = cell_access(m); c = seedmean(m, :constraint); e = seedmean(m, :effective_size)
...
fd["base_cells"] = let out = Dict{String,Float64}[]
    for sub in ("oat", "phase"), (root, _, files) in walkdir(joinpath(ROOT, sub))
        ...
        c = seedmean(m, :constraint); isnan(c) && continue
        push!(out, Dict("rho" => Float64(cfg["rho"]), "delta" => Float64(cfg["delta"]),
                        "constraint" => c, "effsize" => seedmean(m, :effective_size),
...
println("wrote $OUTFILE ($(round(filesize(OUTFILE) / 1024; digits=1)) KB; ",
        length(fd["base_cells"]), " base cells, ", length(fd["oat_cells"]), " OAT cells, ",
        length(fd["grid_cells"]), " grid cells)")
