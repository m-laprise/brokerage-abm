...
Burt's aggregate constraint and Burt's effective size:
  ...
  S4  each measure over time at the baseline pair, no capture vs capture, with a
      legend (the betweenness panel of Figure 5, one panel per measure)
...
const COL_NOCAP = :teal               # no-capture line in S4
const COL_CAP = :darkorange           # capture line in S4
...
const FD = JLD2.load(normpath(joinpath(@__DIR__, "..", "..", "paper", "supp_figdata.jld2")))["figdata"]
const PER = FD["period"]
const SER = FD["series"]
const MARKER_SIZE = 10
savefig(fname, fig) = (save(joinpath(OUT, fname), fig; px_per_unit=PXU); println("  $fname done"))

# measurement-period indices and the smoothed series of `key` for `model`
measidx() = [i for i in eachindex(PER) if PER[i] % MEASINT == 0]
measseries(model, key) = (mi = measidx(); (PER[mi], rolling_mean(SER[model][key][mi], ROLLW)))
...
function ywin(curves...)
    ...
    lo, hi = minimum(v), maximum(v); pad = 0.06 * (hi - lo + eps())
    (lo - pad, hi + pad)
end
...
function supp_S1_grid_lines()
    ...
    for (ci, (key, lab)) in enumerate((CONSTR, EFFS))
        ck = CELLKEY[key]
        ax = Axis(fig[1, ci]; title=lab, xlabel="ρ (complementarity vs quality)",
            ylabel = ci == 1 ? "late-window mean" : "", xticks=[0, 0.3, 0.5, 0.7, 1],
...
function supp_S2_position()
    ...
    for (ri, (key, lab)) in enumerate(((CONSTR), (EFFS)))
        # left: the measure over time, no-capture baseline (no access-fraction series)
        x, y = measseries("base", key)
        axl = Axis(fig[ri, 1]; title = ri == 1 ? "Over time, at baseline" : "",
            xlabel = ri == 2 ? "period" : "", ylabel=lab, titlesize=TITLE_FS, xlabelsize=LABEL_FS,
        ...
        axr = Axis(fig[ri, 2]; title = ri == 1 ? "Across regimes" : "",
            xlabel = ri == 2 ? "access fraction" : "", ylabel=lab, titlesize=TITLE_FS,
...
function supp_S3_advantage()
    ...
    ys = [("Rank correlation gap", [c["rankgap"] for c in bc]),
    ...
    for (ri, (ylab, yv)) in enumerate(ys), (ci, (xlab, xv)) in enumerate(xs)
        ...
        for rv in sort(unique(rho))
            mm = rho .== rv
            scatter!(ax, xv[mm], yv[mm]; color=(RHO_COLORS[rv], 0.8), markersize=MARKER_SIZE,
                strokewidth=0.3, strokecolor=:gray30)
        end
        if ri == 1 && ci == 2
            rvs = sort(unique(rho))
            els = [MarkerElement(marker=:circle, color=RHO_COLORS[v], markersize=MARKER_SIZE) for v in rvs]
...
# ── S4: each measure over time at the baseline pair, no capture vs capture ──
function supp_S4_capture_dynamics()
    fig = Figure(size=(1180, 460))
    for (ci, (key, lab)) in enumerate((CONSTR, EFFS))
        xb, yb = measseries("base", key)
        xc, yc = measseries("capture", key)
        ax = Axis(fig[1, ci]; title=lab, xlabel="period",
            titlesize=TITLE_FS, xlabelsize=LABEL_FS, ylabelsize=LABEL_FS, xticklabelsize=TICK_FS,
            yticklabelsize=TICK_FS, xticks=50:50:200, limits=((TSTART, 201), ywin((xb, yb), (xc, yc))))
        scatterlines!(ax, xb, yb; color=COL_NOCAP, linewidth=2.2, markersize=6, label="no capture")
        scatterlines!(ax, xc, yc; color=COL_CAP, linewidth=2.2, markersize=6, label="capture")
        ci == 1 && axislegend(ax; position=:rt, LEG_KW...)
    end
    colgap!(fig.layout, 16)
    savefig("supp_S4_capture_dynamics.png", fig)
end

for (name, f) in (("S1", supp_S1_grid_lines), ("S2", supp_S2_position),
                  ("S3", supp_S3_advantage), ("S4", supp_S4_capture_dynamics))
    try
        f()
    catch e
...
open(normpath(joinpath(@__DIR__, "..", "..", "paper", "supp_figmeta.tex")), "w") do io
    println(io, "% supp_figmeta.tex: generated by scripts/paper/supp_figures.jl. Do not edit by hand.")
    println(io, "% Display conventions used to render paper/supp_figs/, quoted in captions via \\pv keys.")
