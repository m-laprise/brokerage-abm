...
function has_final_agent_degrees(final_agent_degrees, n_seeds::Int, N::Int)
    final_agent_degrees isa Vector || return false
    length(final_agent_degrees) == n_seeds || return false
    return all(
        degrees -> degrees isa Vector{Int} && length(degrees) == N, final_agent_degrees
    )
end
...
function plot_network_stats(
    mdfs::Vector{DataFrame},
    final_agent_degrees::Vector{Vector{Int}},
    suptitle::String,
    filename::String;
    T_burn::Int=30,
    window::Int=20,
    ...
    time_axes = Axis[]
    new_time_ax(pos; kw...) = (a=Axis(pos; kw...); push!(time_axes, a); a)

    Label(
        fig[0, 1:4],
        "Agent network statistics: $suptitle";
        fontsize=SUPTITLE_FS,
        font=:bold,
        halign=:center,
        tellwidth=false,
    ...
    pooled_final_degrees = reduce(vcat, final_agent_degrees)
    max_final_degree = maximum(pooled_final_degrees)
    hist_ax = Axis(
        fig[1, 4];
        title="Final degree distribution",
        xlabel="Degree",
        ylabel="Agent count",
        limits=((0, max_final_degree + 1), (0, nothing)),
        titlesize=TITLE_FS,
        xlabelsize=LABEL_FS,
        ylabelsize=LABEL_FS,
    ...
    hist!(
        hist_ax,
        pooled_final_degrees;
        bins=0:(max_final_degree + 1),
        color=(COL_DIAG, 0.65),
        strokewidth=0.5,
        strokecolor=:gray35,
    )
...
function plot_dgp_matrix(F; tag, rho, delta, s, N)
    crange = let m = maximum(abs, F .- mean(F));
        (mean(F) - m, mean(F) + m)
    end
    fig = Figure(; size=(800, 650))
    Label(
        fig[0, 1:2],
        "Output matrix (N=$N, ρ=$rho, δ=$delta, s=$s)";
        fontsize=SUPTITLE_FS-2,
        font=:bold,
        halign=:center,
        tellwidth=false,
    )
    ...
    ax2 = Axis(
        fig[2, 1]; xlabel="q", ylabel="Count", xlabelsize=LABEL_FS, ylabelsize=LABEL_FS
    )
...
function plot_dgp_svd(F; tag, rho, delta, s, N)
    ...
    Label(
        fig[0, 1:2],
        "SVD of output matrix (ρ=$rho, δ=$delta, s=$s)";
        fontsize=SUPTITLE_FS-2,
        font=:bold,
        halign=:center,
        tellwidth=false,
...
configs = [
    (tag="baseline", label="Baseline (ρ=0.50, δ=0.50, s=8, η=0.02)", kwargs=(;)),
    # rho sweep
    (tag="rho00_pureinteraction", label="Pure interaction (ρ=0.0)", kwargs=(rho=0.0,)),
    (tag="rho10_weakquality", label="Weak quality (ρ=0.10)", kwargs=(rho=0.10,)),
    (tag="rho30_mildinteraction", label="Mild interaction (ρ=0.30)", kwargs=(rho=0.30,)),
    (tag="rho70_mildquality", label="Mild quality (ρ=0.70)", kwargs=(rho=0.70,)),
    (tag="rho90_strongquality", label="Strong quality (ρ=0.90)", kwargs=(rho=0.90,)),
    (tag="rho100_purequality", label="Pure quality (ρ=1.0)", kwargs=(rho=1.0,)),
    # delta sweep
    (tag="delta00_noregime", label="No regime effect (δ=0.0)", kwargs=(delta=0.0,)),
    (tag="delta25_weakregime", label="Weak regime (δ=0.25)", kwargs=(delta=0.25,)),
    (tag="delta75_strongregime", label="Strong regime (δ=0.75)", kwargs=(delta=0.75,)),
    # s sweep
    (tag="s2_lowdim", label="Low-dim curve (s=2)", kwargs=(s=2,)),
    (tag="s4_middim", label="Mid-dim curve (s=4)", kwargs=(s=4,)),
    # eta sweep
    (tag="eta01_stable", label="Stable market (η=0.01)", kwargs=(eta=0.01,)),
    (tag="eta05_volatile", label="Volatile market (η=0.05)", kwargs=(eta=0.05,)),
]
...
for (idx, c) in enumerate(configs)
    ...
    if !RERUN && isfile(datafile)
        ...
        mdfs = saved["mdfs"]
        final_agent_degrees = get(saved, "final_agent_degrees", nothing)
        if cache_degree_stats_version(saved) != NETWORK_DEGREE_STATS_VERSION ||
            !has_network_degree_stats(mdfs) ||
            !has_final_agent_degrees(final_agent_degrees, N_SEEDS, N_SIM)
            println("  Cache has stale degree summaries, re-simulating")
            result = run_ensemble(;
                base_kwargs=c.kwargs,
                T=T,
                N=N_SIM,
                n_seeds=N_SEEDS,
                return_final_agent_degrees=true,
            )
            mdfs = result.mdfs
            final_agent_degrees = result.final_agent_degrees
            jldsave(
                datafile;
                mdfs=mdfs,
