#!/bin/bash
# Run one Ridge calibration task from a stage manifest.
...
export BROKERAGE_ABM_RIDGE_CALIBRATION_STAGE_DIR="${2:?missing stage directory}"
shift 2
...
cd "$REPO"
echo "task=${SLURM_ARRAY_TASK_ID:-manual} host=$(hostname) stage=$BROKERAGE_ABM_RIDGE_CALIBRATION_STAGE_DIR"
julia --compiled-modules=strict --pkgimages=existing --project \
    --threads="${SLURM_CPUS_PER_TASK:-2}" scripts/ridge/calibration/run.jl "$@"
