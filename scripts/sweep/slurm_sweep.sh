...
export TB_SWEEP_DIR="${2:?usage: slurm_sweep.sh <repo_root> <sweep_dir>}"
...
cd "$REPO"
echo "task=${SLURM_ARRAY_TASK_ID} host=$(hostname) cpus=${SLURM_CPUS_PER_TASK} sweep=$TB_SWEEP_DIR"
