...
while true; do
    ts=$(date '+%F %T')
    NCONDITIONS=0; NGRIDCELLS=0; NRUNS=0; NPLOT=0
    ...
    printf '[%s] design R%d/G%d | runs %d/%d (%d%%)  +%d/%ss  ETA~%s | queue run R%d/P%d  plot R%d/P%d | data %d  png %d\n' \
        "$ts" "$NCONDITIONS" "$NGRIDCELLS" "$shards" "${NRUNS:-0}" "$pct" \
        "$d" "$INTERVAL" "$eta" "$run_R" "$run_P" "$plt_R" "$plt_P" \
        "$data" "$png" | tee -a "$LOG"
