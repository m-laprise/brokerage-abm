#!/bin/bash
# Dependent plotting array: one task per OAT cell or per (phase pair, model).
# Submit after the compute array with a dependency + array range + log paths:
#   sbatch --dependency=afterany:<compute_jobid> --array=0-48%24 \
...
export TB_SWEEP_DIR="${2:?usage: slurm_plot.sh <repo_root> <sweep_dir>}"
...
cd "$REPO"
echo "plot task=${SLURM_ARRAY_TASK_ID} host=$(hostname) cpus=${SLURM_CPUS_PER_TASK} sweep=$TB_SWEEP_DIR"
